...
PRESETS = {
    ...
    "meguri": dict(windows=[(6 * 60 + 30, 8 * 60 + 30),    # 朝 6:30-8:30（縁起日告知）
                   ...
                   min_gap_min=30),
    # seizogyo2（住田・共感認知型）＝朝夜寄り。seizogyo3（ぱし・本音暴露型）と窓が重ならないよう配置し、
    # 2アカ(同一運用者)の投稿ニアミス＝CIBシグナルを避ける（アカウントごとに別シート＋別プリセット）。
    "seizogyo2": dict(windows=[(6 * 60 + 30, 8 * 60),        # 朝 6:30-8:00（通勤）
                               (11 * 60 + 30, 12 * 60 + 45), # 昼 11:30-12:45（休憩前半）
                               (17 * 60 + 30, 18 * 60 + 45), # 夕 17:30-18:45（帰宅直後）
                               (22 * 60, 23 * 60 + 30)],     # 夜 22:00-23:30（寝る前）
                      min_gap_min=30),
    "seizogyo3": dict(windows=[(9 * 60, 10 * 60 + 30),       # 朝 9:00-10:30（通勤後・住田朝の後）
                               (13 * 60 + 15, 14 * 60 + 30), # 昼 13:15-14:30（遅い昼）
                               (16 * 60 + 30, 17 * 60 + 20), # 夕 16:30-17:20（夕方前半）
                               (20 * 60 + 5, 20 * 60 + 55)], # 夜 20:05-20:55（住田の夕と夜の谷間）
                      min_gap_min=30),
}
