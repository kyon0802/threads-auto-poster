...
>
> ⚠️ **未処理の宿題（09-03の日程重複）**：復旧のため 08-31（サイクル日ではない日）に `FORCE_CYCLE=1` で
> 回したため、生成分が 09-01〜09-03 に載った。次の自然サイクル **09-02** は 09-03〜09-05 を生成するので、
> **09-03 が1日8本（各アカ）に二重予約される**。`MAX_POSTS_PER_DAY=5` で溢れた分は overdue となり、
> 翌日に連射される（§10のスパム判定リスク）。**09-02 のサイクル実行前に、09-03 の既存4本を
> `retired` にすること**。原因は generator が既存在庫を見ず常に「翌日から3日分」を生成する設計
> （`build_schedule` の `not_before` は過去スロット除外専用で、埋まっている日を避けない）。
>
