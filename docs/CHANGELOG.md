...
### §28追補（2026-08-31）復旧完了 — 4アカウント全て再稼働

クレジット購入により当日中に復旧。preflight → 週次強制実行 → 在庫確認まで一気に通した。

**復旧の手順（次に同じことが起きたときの正本）**
1. `preflight.yml` 手動実行 → `✅ 正常：生成AIは使えます` を確認。
   （購入直後2回はまだ反映前で「残高不足」のまま。数分後の3回目で緑になった＝**購入直後は反映待ちがある**）
2. Variable を設定：`GEN_POSTS_SEIZOGYO2/3/MEGURI=12`（生成ON）＋ `FORCE_CYCLE=1`（サイクル日以外に回すため）
3. `weekly.yml` 手動実行（5m10s・成功）
   → **4アカ×12本＝48本を生成、機械コンプラゲート 合格48/破棄0、queued投入、メール4通、失敗0**
4. **`FORCE_CYCLE=0` に即戻す**（1のままだと毎日48本生成してしまう。ここが一番の事故ポイント）
5. `monitor.yml` 手動実行 → 全4アカ「在庫12本（残り3日）」・**1ヶ月ぶりに緑**（exit 0）

**停止期間の最終値**：takumi 43日／住田(tenshokuman) 53日／ぱし(pashi) 49日／澪(mio__meguri) 21日。

**判断の記録**：オーナー判断で `GEN_STATUS=queued`（無確認の自動公開）のまま継続し、
seizogyo2/seizogyo3/meguri も含め**4アカウント全ての自動生成をON**にした
（draft運用・段階復旧の選択肢を提示した上での決定）。

### ★積み残し：09-03 の日程重複（09-02 のサイクル実行前に要対応）

**現象**：復旧のため 08-31（＝サイクル日ではない日）に `FORCE_CYCLE=1` で回したため、
生成分が **09-01〜09-03** に載った。次の自然サイクル日は **09-02** で、そこでは
**09-03〜09-05** が生成される。よって **09-03 が各アカ8本（4+4）に二重予約される**。

**影響**：`MAX_POSTS_PER_DAY=5` で溢れた3本は overdue として残り、翌日の最初の実行で
連射される（単発投稿の間にウェイトが無い＝§10のスパム判定リスク。製造業はBAN歴2回）。

**原因（設計上のギャップ）**：generator は既存在庫を見ず、常に「翌日から3日×4本」を生成する。
`build_schedule` の `not_before` は**過去スロットの除外専用**で、既に埋まっている日を避ける機能は無い。
3日サイクルを厳守している限り重複しない前提の設計であり、`FORCE_CYCLE` での臨時実行は
この前提を崩す。**`FORCE_CYCLE=1` を使うときは常にこの重複を確認すること。**

**対応（どちらか）**：
- (a) 09-02 のサイクル実行前に、各シートの `投稿_<acc>` で **09-03 の既存4本を `retired`** にする
  （未来日時の行なので §10 の「今すぐ公開になる行を触らない」に抵触しない安全な操作）。
  → 在庫は 09-01・09-02 の8本となり、09-02サイクルの 09-03〜09-05 と隙間なく連続する。
- (b) 恒久対策として generator に「既に埋まっている日をスキップする」機能を入れる（未実装）。
...
### §29追補（2026-09-01）シート移行の実施＋09-03二重予約の解消

- **シート移行実施**：`scripts/add_pdca_columns.py` を4事業シートに DRY-RUN→`--apply` で実行。
  各 `投稿_<acc>` に3列（フック型/内容型/参照お手本ID）追加、`お手本DB_<acc>`・`仮説ログ` を作成。
  再実行で全スキップ（冪等）を実測確認。シートIDはコマンドライン引数で渡し repo には残していない（§17b）。
- **09-03二重予約の解消（§28積み残し）**：08-31復旧バッチの 09-03 分（各アカ4本・計16行
  `*-g20260831-09〜12`・全て queued・未来日時）を row_id 指名＋日時/状態の三重チェック付きで
  `retired` 化し、再サーベイで16/16の反映を確認。09-02 サイクルは 09-03〜09-05 をクリーンに生成する。
- 廃止済み `uranai`（結）のシートは移行対象外（BUSINESSES 外・生成なし）。

### §29追補2（2026-09-03）運用監査で見つかった3問題の根本対処

09-02 サイクル後の実測監査（公開32本すべて投稿後ID付与・48本生成・型ラベル48/48記録・在庫3日）で、
稼働は正常だったが次の3点が見つかった。いずれも根本対処し、テスト97→108本。

- **A. 型ラベル語彙のアカウント別化**：澪の9/2生成分12本中6本が汎用6型に無い名前だった。原因は
  コードの `HOOK_TYPES` が製造業向けの汎用固定で、澪はナレッジ§4に占い専用6型（縁起日×時間限定／
  知的な常識否定／防御線・逆説／ラベリング問いかけ／承認・許可／断定の告知）を持ち、AIがそれに正しく
  従ったこと。対処＝`プロフィール_<acc>` の項目「フック型語彙」を語彙の正本にし（無ければ汎用6型）、
  生成の構造化出力で **enum 強制**（自由命名を廃止。探索枠＝サンプルの少ない型を試す、に定義変更）。
  澪のプロフィールに6型を登録し、本番経路（get_profile→hook_types_for）で読み戻しを確認。
  生成関数は run() 時にアカウント語彙を焼いて構築（注入された旧シグネチャ `fn(prompt)` も互換）。
- **B. Google Sheets 429 対策**：①`_ws_by_title()` でタブ一覧をインスタンス内キャッシュ（従来は
  タブを読むたびに `worksheets()` を呼び、第1段のお手本DB読み取り追加で悪化）②`with_retry` を
  5回→6回（2,4,8,16,32秒＝合計62秒）にし「1分あたり」クォータを確実に跨ぐ（8/29の実障害は
  合計30秒で跨げなかった）。
- **C. 在庫を見て生成**：`Generator.run(existing_posts=...)` で既存の未来在庫（queued/draft/
  publishing/空。retired・posted・他アカは除外）の最終日時を起点にし、その翌日から予約する。
  main_weekly は読み済みの `posts_all` を渡す（追加読み取りなし）。これで `FORCE_CYCLE=1` の
  臨時実行でも次サイクルと重複しない（09-01の16行手動退避の再発防止）。

残る人側の作業：Anthropic の Auto-reload 設定確認／第3段前の `nlm login`／お手本スクショの投下。

## §30 改修ログ（2026-09-07）殿堂入り＋傾向分析3窓＋重複ゲート

設計= docs/superpowers/specs/2026-09-07-hall-of-fame-design.md

### 発端：「データは蓄積されているか」の実測

全タブが追記/upsert で、過去データは消えていないことを確認（takumi インサイト7,958行・
ユニーク161投稿・6/15の投稿も健在・スナップショット76日分で欠けなし）。`.clear()` するのは
`インサイト分析_<acc>`（派生集計20行・再計算可能）のみ。**蓄積は正常**。

**問題は「貯めているのに使っていない」こと**。生成AIに渡る勝ち投稿は28日窓のTOP3だけで、
`lifetime` は合計値しか持たず本文が無かった。実測の取りこぼし：takumi は自己ベスト
（3,759表示・6/18）を、澪は上位5本すべてをAIに一度も見せないまま次を作っていた。

### 対処1: 殿堂入り（`hall_of_fame.py`・`殿堂入り_<acc>` タブ）

- 全期間の**表示回数 上位30本**をタブにDBとして持つ（オーナー決定：選定は表示回数のみ）
- `保護` 列が非空の行は自動更新で温存＝人が手で入れた投稿を消さない
- プロンプトへは**上位10本内で日付ローテーションして3本**。30本を平等に回すと「23位」の
  ような弱い実例を見せる回が出て「当たりに寄せる」目的から外れるため、プールを上位に限定
- 週次レポートには出さない（オーナー決定）。生成日のみ更新＝追加のシート読み取りなし

### 対処2: 傾向分析を3窓に（60日 / 180日 / 全期間）

主軸を28日→**60日**に広げてサンプルを増やし、180日と全期間を併記。
`インサイト分析_<acc>` に `時間帯(60日)` のように窓名つきで並べ、人が開いて取り違えないようにした。

### 対処3: ★重複ゲート（緊急・BANリスク）

**2026-09-01のPDCA機能（勝ち投稿の本文をAIに見せる）を入れた直後から、AIが実例を丸写しし、
完全一致の投稿が数日おきに再投稿されていた。**

| アカウント | PDCA前 | PDCA後(9/1〜) |
|---|---|---|
| 住田 | 12本中 0組 | 26本中 **11組** |
| ぱし | 12本中 0組 | 26本中 **15組** |
| takumi | 137本中 7組(0.08%) | 25本中 3組(1.0%＝13倍) |

Threadsで同一本文を繰り返すのはスパム判定の引き金で、製造業は過去2回BAN。
投稿はAPIで削除できない（§3）ため公開されたら取り返しがつかない。

- `compliance.find_near_duplicate()`：既存の公開/予約/**同一バッチ内**と類似0.85以上の生成を破棄
  （difflib。quick_ratio で安くふるってから ratio で確定）。プロンプト側にも丸写し禁止を明記
- 実データ再生で**重複38本を阻止・正常253本は通過**を確認
- 未公開だった重複予約3本のうち、未来の2本（住田17:55・澪18:00）を `retired` 化。
  ぱしの1本（16:35）は発見の4分前に公開済みで間に合わなかった

### 教訓

「勝ち投稿の実物をAIに見せる」施策は、**丸写しを防ぐ機械ゲートとセットでなければ危険**。
プロンプトの禁止文言だけでは守られない（実際に守られなかった）。

---

## §31 改修ログ（2026-09-08）外注アカウント対応 — 収集と外注管理のみ、投稿・生成は機械で禁止

### 背景

自社4アカ（takumi / 住田 / ぱし / 澪）に加え、**外注先が運用する2アカウント**をシステムに載せた。
これまでシステム上に存在せず、実績の把握は手作業のCSV分析に頼っていた。

用途が自社アカと根本的に違う。**投稿を書くのは外注先**であり、こちらの役割は
「実績の収集」と「外注の作業量の監督」だけ。同じ扱いにすると次の事故が起きる。

1. **外注先のアカウントに勝手に投稿してしまう**（最も重大）
2. AI生成が走り、誰も公開しない投稿にAPI課金が発生。外注先の投稿とも二重になる
3. 在庫監視が毎日 critical → Actions が赤く固定され、**本物の停止を見逃す**（§27 の再来）

### 対処: `accounts` タブに「運用種別」列を追加

空 or `自社` ＝ 従来どおり。`外注` ＝ 外注アカ。**空欄が自社なので既存4アカのシートは無変更で動く。**

| 入口 | 自社 | 外注 |
|---|---|---|
| `post.yml` 投稿 | する | **しない** |
| `post.yml` トークン更新 | する | **する**（60日で失効すると収集が止まるため） |
| `collect.yml` 収集 | する | **する**（主目的） |
| `weekly.yml` AI生成 | する | **しない** |
| `weekly.yml` レポート | 従来の週次 | **外注作業量レポート**（2アカを1通に） |
| `monitor.yml` 在庫監視 | する | **しない** |

未知の値（打ち間違い）は**自社扱い**。外注扱いに倒すと自社アカの投稿が全停止するのに対し、
逆は実害が小さい（外注アカには投稿タブ `投稿_<acc>` を作らないため公開対象の行が存在しない）。

### 対処: インサイトに「本文」列を追加

使い回し率は本文の照合でしか出せない。**Threads API の投稿一覧は元から本文を返しており**
（`threads_api.list_media` の fields に `text` あり）、`collector.py` が本文長だけ保存して
本文を捨てていた。列を1つ足すだけで済み、**API 呼び出しは増えない**。

### 対処: 外注管理指標（`threads_poster/vendor.py`・新規）

`analyzer.py`（自社の勝ちパターン抽出）とは目的が違うので分離。2026-09-08 の実測分析
（外注2アカ・33日・151投稿）で実際に判断材料になった指標だけを採った。

- **新しく書いた本文の数**（＝実際の制作量。実測では151投稿中91本）
- **使い回し率**（実測40%・直近週は78%）
- 稼働日数 / 無投稿日 / 投稿の多い時間帯 / 表示の合計・中央値・表示5以下

集計上の落とし穴を2つテストで固定した。
**(1)** インサイトは `posted_id × 取得日` で毎日1行積まれるため、素直に数えると1本の投稿が
日数ぶんに膨らみ作業量を過大評価する → 投稿単位に畳んで最新スナップショットを採用。
**(2)** 本文列を足す前に収集された行は本文が空。重複と数えると使い回し率が100%に見える →
分母から外し `text_missing` として件数を明示。

### テスト

`tests/test_vendor_accounts.py`（新規）で、事故の3つを機械で禁止した。
「外注アカに投稿しない」は**実際に投稿されることを確認してから**修正している（RED→GREEN）。
全体 166本パス。

### 人がやること

1. 外注先に Threadsテスター追加 → 認可 → `scripts/get_auth_url.py` → `scripts/exchange_token.py`
2. `python3 scripts/add_vendor_columns.py --sheet-id <seizogyoのID> --apply`
3. `accounts` タブに2行追加（運用種別＝`外注`・投稿タブは作らない）

設計の全文 → `docs/superpowers/specs/2026-09-08-vendor-accounts-design.md`

### 追記（2026-09-09）先行設計との整合と、中央値を主指標にする修正

本改修の実装後に、**先行する設計書 `docs/superpowers/specs/2026-09-06-cross-account-analytics-design.md`
（横断分析基盤・設計合意済み）が同じ問題を扱っていた**ことが判明した。実装時にこれを確認しておらず、
置き場所の方針が分かれた（先行設計＝分析専用の別シート／本実装＝seizogyo シートに相乗り）。

オーナー判断により、**本実装を第1段として残し、先行設計側に実装状況を追記して整合させた**。
先行設計の3段のうち ①曜日固定は実装済み、②トークン共通化は未実装だが
publisher が外注アカのトークンも更新するため 60日失効の問題は起きず優先度が下がった。
③のうち「自社との横断比較・横断ダッシュボード・お手本DBへの還元」は未実装として残る。

あわせて先行設計の**必須要件**を取り込んだ。

> 連携直後に委託先アカウントの1ヶ月分を実測したところ、**平均表示と中央値表示で評価が逆転する**
> ケースが実際に出た。1本の突出した投稿が平均を押し上げ、残り半数が表示ひと桁という分布である。

平均だけで判断すると外注先への指示が逆になるため、`vendor.py` に `views_mean` を追加し、
レポートでは**中央値を先に「実態」として出し、平均は乖離を見せる併記**に留めた。テスト2本を追加（全体168本）。

### 未確定（2026-09-09）

2026-09-07 のセッションで外注2アカのAPI連携まで到達していた形跡がある（取得CSVに
Threads インサイトAPI の指標一式が揃っている）が、**そのトークンがシステムの読むシートに
登録されていない**。稼働にはトークンの所在確認が先決。

### 追記（2026-09-09）トークン取得を外注アカ用にやり直せるようにした

9/7に取得したトークンの所在が確認できなかったため、再取得の導線を整備した。
あわせて先行設計の「やらないこと」を**トークンの権限レベルで**満たすようにした。

- `scripts/get_auth_url.py` に `--collect-only` を追加。`threads_content_publish` を
  **要求しない**認可URLを出す。コード側（publisher）でも投稿を止めているが、トークン自体に
  権限が無ければ万一分岐をすり抜けても API が投稿を拒否する＝二重の防御
- `scripts/setup_account.py` に `--role`（運用種別）と `--sheet-id` を追加。
  外注アカは相乗り先の事業シートを明示して登録する（環境変数 `SPREADSHEET_ID` は
  旧単一シートを指しており、そのまま使うと登録先を間違える）
- 同スクリプトが**運用種別の打ち間違い**と**外注アカへの `--add-test-post`** を入口で拒否する。
  前者は `is_outsourced()` が未知値を自社扱いに倒す設計のため、入口で止めないと
  「外注のつもりが自社で登録」され、投稿対象になってしまう

テスト5本追加（全体173本）。

`scripts/exchange_token.py` に `--out` を追加。長期トークンを**画面に出さず**
権限0600のファイルへ保存し、そのまま `setup_account.py --token-file` へ渡せるようにした
（作成時から0600で開き、権限設定までの間に他人が読める瞬間を作らない）。テスト2本追加（全体175本）。

### 追記（2026-09-11）トークンは既存を流用してよい／棚卸しツールを追加

**判断の変更**: 「分析専用アカウントのトークンに投稿権限を付与しない」は**必須ではなく多層防御の1枚目**
と位置づけ直した。投稿を止めているのはコード側（`publisher` が運用種別=外注をスキップ・テストで固定）で、
さらに外注アカには投稿タブ `投稿_<acc>` を作らないため公開対象の行がそもそも存在しない。
収集に必要な権限は投稿権限つきトークンにも含まれるので、**既に発行済みのトークンをそのまま使える**。
2種類を発行し分ける運用コストのほうが割に合わない。`--collect-only` は**取り直すときの推奨**として残す。

**`scripts/check_accounts.py`（新規・読み取り専用）**: 「前回発行したトークンがまだ使えるのか」
「どのシートに登録済みなのか」を確かめる手段が無く、分からないなら取り直すしかない状態だった。
長期トークンは60日有効なので、生きていれば認可からやり直す必要はない。

- `threads_poster/accounts_status.py`（純関数）が accounts の行から
  経過日数・残り日数・状態（ok/warn/expired/missing/unknown）を算出する
- **トークンの値は戻り値にも画面にも出さない**（§17b）。テストで固定
- `--verify` で Threads API の `GET /me` と投稿一覧を1回ずつ叩き、実際の生死と
  収集用の権限があるかを確認する（読み取りのみ・投稿しない）

テスト6本追加（全体181本）。

**同日の誤警告と修正**: `check_accounts.py --verify` が本番の主力アカ `takumi_kojo_navi` について
「収集用の権限が不足」と表示した。実際は権限に問題はなく、`graph.threads.net` の一時的な
read timeout を例外で拾い、権限不足と同じ扱いにしていただけだった。
誤警告は「本物の権限不足」を疑わせて不要なトークン取り直しを招くため、
`classify_read_result()`（純関数）で**通信エラーと権限不足を必ず言い分ける**ようにした。
タイムアウトも30秒→60秒へ。テスト4本追加（全体185本）。

### 稼働開始（2026-09-11）外注1アカ目を登録

外注アカの1つ目を seizogyo シートに **運用種別=外注** で登録した（アカウント名は第三者のものなので
公開repoには書かない＝§17b。実名はシートと非公開メモリにある）。

- 認可はブラウザ（本人がログイン済み）で `--collect-only` のURLを開いて承認。
  **投稿権限を含まないトークン**（threads_basic + threads_manage_insights のみ）を取得
- 同意画面に「あなたは以前、あなたのThreadsアカウントに自動投稿をリンクしました」と表示され、
  2026-09-07 に認可済みだったことも確認できた（そのトークンは登録されないまま所在不明だった）
- 4事業シート全てに `運用種別`・`本文` 列を追加（データは無傷。takumi のインサイト8,170行も保持）
- `main_collect.py` の DRY-RUN で **インサイト84件・エラー0件**の取得を実測確認

**もう1アカは未登録。** そのアカウントでブラウザにログインした状態で
同じ手順（認可URL→承認→交換→登録）を踏めば入る。

### 完了（2026-09-11）外注2アカとも稼働開始 — 6アカ運用へ

外注2アカとも登録し、実データでの動作を確認した。**自社4＋外注2＝6アカ運用**になった。

| 確認項目 | 結果 |
|---|---|
| 登録 | 2アカとも `accounts` に 運用種別=`外注` で追加（seizogyo シート） |
| トークン | 2アカとも60日有効・**投稿権限なし**（`threads_basic`＋`threads_manage_insights`） |
| 収集 | 3アカ238件・**エラー0件**。外注2アカのインサイトタブが自動生成され**各84行** |
| 本文の保存 | **84行すべてに本文あり**（使い回し率の算出に必要） |
| レポート | 実データから外注作業量レポートの生成を確認 |

収集した数字が **2026-09-08 に手作業でCSV分析した結果と一致**した（最多表示2,063／682など）ので、
パイプライン全体が正しいことの裏付けにもなった。

**直近7日の実測**: 制作量は回復（使い回し 0%／8%・9/2〜9/6は78%だった）が、
**表示は低いまま**（中央値2〜6・表示5以下が最大9本）。投稿が 10時・21時・0時 に寄っており、
09-08の分析で最も弱かった時間帯にあたる（強いのは12〜16時）。**次の論点は投稿時間帯**。

#### この改修で分かった運用上の落とし穴（2つ）

**① 状態がどこにも書かれていないと、次のセッションが同じ作業をやり直す。**
2026-09-07 に外注アカのAPI連携とデータ取得を実施していたが、トークンをシートに登録しないまま
終わり記録も残らなかったため、翌日は「未着手」と誤認して認可からやり直した。
対策として CLAUDE.md §2 に **「状態を推測しないこと」** と実測コマンドを明記した。

**② ローカルの `.env` に `DRY_RUN=1` が入っている。**
`set -a; . ./.env; set +a` で読み込むと以降が全て無書込になるが、ログは
`収集結果: {...errors: 0}` と**成功に見える**。実際に「本番実行した」と誤認し、
シートを見て初めて1行も書かれていないことに気づいた。CLAUDE.md §10 に追記。
GitHub Actions は `.env` を読まないため本番の自動運用には影響しない。

#### トークンの取り扱いを明文化（CLAUDE.md §17b）

トークンの正本は**非公開シートの `accounts` タブだけ**。ローカルのメモ・repo（公開/非公開問わず）・
GitHubの「外から見えない場所」・チャット本文・ターミナル画面のいずれにも残さない。
取得時は `--out` で0600の一時ファイルに落とし、登録したら即削除する。

## §32 改修ログ（2026-09-11）全投稿の閲覧用シートを自動更新する

### 背景

投稿データは事業ごと・アカウントごとにシートが分かれ、しかもインサイトは
`posted_id × 取得日` の日次スナップショットで積まれるため、**人が「全アカの投稿を月別に眺める」
ことができなかった**。都度こちらで集計してファイルを作る運用は続かない。

### 対処: `main_export.py` ＋ `export.yml`（日次05:00・読取専用）

全事業シートを読み、**1枚の閲覧用シートを毎日全置換**する。タブ構成は
`読み方` / `サマリ_月別` / `<アカウント名>×7` / `全投稿`。

- **1投稿1行に畳む**（最新スナップショットを採用）。素直に出すと1投稿が日数ぶんに膨らむ
- **本文を投稿タブから補完**する。本文列は 2026-09-11 追加のため古い行は空で、
  自社アカはこちらが書いた原稿から復元できる（実測で283件を復元）
- **月別サマリは中央値が主指標**、平均は併記（2026-09-06 横断分析設計の必須要件）
- Threads API も AI も叩かない＝課金も投稿も発生しない

### ★サービスアカウントはファイルを新規作成できない

`gspread.create()` は `403 The user's Drive storage quota has been exceeded` で失敗する。
サービスアカウントは Drive 容量を持たないため、**自分が所有者になるファイルを作れない**。
よって **シートの器は人が作り、サービスアカウントに編集権限で共有し、そのIDを
Secret `EXPORT_SHEET_ID` に入れる**という分担にした。中身の作成と更新はジョブが行う。

初回は 7アカウント・760投稿を書き出して動作を確認済み。

### 追記（2026-09-11）廃止アカウントの過去データも閲覧用シートに含める

初回の本番実行は 6アカ604本で、**廃止済みアカウント（占い「結」・156本）が抜けていた**。
本番の `BUSINESSES` に廃止シートが入っていないため。ただし BUSINESSES に足すと
post/weekly/monitor まで動き出してしまうので入れられない。

そこで **閲覧用シートにだけ含める読み取り専用の追加指定** `EXPORT_EXTRA_SHEETS` を設けた
（`BUSINESSES` と同じJSON形式）。ここに入れたシートは書き出しの対象にはなるが、
投稿・生成・在庫監視の対象にはならない。同じシートIDが重複したら1回だけ読む。
指定が壊れていても通常分の書き出しは止めない。テスト4本追加（全体196本）。
